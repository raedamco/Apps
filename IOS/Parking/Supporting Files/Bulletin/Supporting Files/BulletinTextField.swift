...
class TextFieldAddDataBulletinPage: FeedbackPageBLTNItem {
...
extension TextFieldAddDataBulletinPage: UITextFieldDelegate {
    ...
    func sendRequest(Text: String!){
        functions.httpsCallable("addVehicleData").call(["UID": UserData[indexPath.row].UID, "VehicleData": Text]) { (result, error) in
//            if let error = error as NSError? {
//                if error.domain == FunctionsErrorDomain {
//                    functionError = true
//                    let message = error.localizedDescription
...
//            }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        if isInputValid(text: textField.text){
            sendRequest(Text: textField.text)
        }else{
            descriptionLabel!.textColor = .red
            descriptionLabel!.text = "You must enter a licence plate to continue"
            textField.backgroundColor = UIColor.red.withAlphaComponent(0.3)
        }
    }
}
